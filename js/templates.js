  ...
  renderSectionsInOrder(d, styles, tplName) {
    ...
    order.forEach(sectionKey => {
      switch (sectionKey) {
        case 'summary':
          if (d.summary) {
            htmlContent += this.sections[tplName].summary(d.summary, styles.accentColor);
          }
          break;
        case 'experience':
          if (d.exps && d.exps.length > 0) {
            htmlContent += this.sections[tplName].experience(d.exps, styles.accentColor, this.renderBullets);
          }
          break;
        case 'education':
          if (d.edus && d.edus.length > 0) {
            htmlContent += this.sections[tplName].education(d.edus, styles.accentColor);
          }
          break;
        case 'skills':
          if ((d.techSkills && d.techSkills.length > 0) || (d.softSkills && d.softSkills.length > 0)) {
            htmlContent += this.sections[tplName].skills(d.techSkills, d.softSkills, styles.accentColor);
          }
          break;
        case 'projects':
          if (d.projs && d.projs.length > 0) {
            htmlContent += this.sections[tplName].projects(d.projs, styles.accentColor);
          }
          break;
        case 'certs':
          if (d.certs && d.certs.length > 0) {
            htmlContent += this.sections[tplName].certs(d.certs, styles.accentColor);
          }
          break;
        case 'extras':
          if (d.langs || d.hobbies || d.achievements) {
            htmlContent += this.sections[tplName].extras(d.langs, d.hobbies, d.achievements, styles.accentColor);
          }
          break;
        case 'custom':
          if (d.customEnabled && d.customTitle && d.customBody) {
            htmlContent += this.sections[tplName].custom(d.customTitle, d.customBody, styles.accentColor, this.renderBullets);
          }
          break;
      }
    });
